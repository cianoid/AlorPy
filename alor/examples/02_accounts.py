...
def main():
    # Счета будет хранить в виде списка
    ap_providers = [
        (settings.user_name, settings.refresh_token),
        # (settings_iia.user_name, settings_iia.refresh_token),  # ИИС
        # (settings_demo.user_name, settings_demo.refresh_token),  # Демо счет для тестов
    ]

    ap_provider = Alor(ap_providers[0][0], ap_providers[0][1])  # Подключаемся к торговому счету
    ...
    for user_name, refresh_token in ap_providers:  # Пробегаемся по всем счетам
        ...
        ap_provider = Alor(user_name, refresh_token)  # Подключаемся к счету
        ...
        for p in portfolios:  # Пробегаемся по всем портфелям
            ...
            for exchange in ap_provider.exchanges:  # Пробегаемся по всем биржам
                ...
                print(
                    f'  - Позиции {round(money["portfolio"] - money["cash"], 2)} + Свободные средства {money["cash"]} ='
                    f' {round(money["portfolio"], 2)}'
                )
                ...
                for order in orders:  # Пробегаемся по всем активным заявкам
                    print(
                        f'  - Заявка номер {order["id"]} {"Покупка" if order["side"] == "buy" else "Продажа"} '
                        f'{order["exchange"]}.{order["symbol"]} {order["qty"]} @ {order["price"]}'
                    )
                stop_orders = ap_provider.get_stop_orders(
                    portfolio_name, exchange
                )  # Получаем список активных стоп заявок
                for stop_order in stop_orders:  # Пробегаемся по всем активным стоп заявкам
                    print(
                        f'  - Стоп заявка номер {stop_order["id"]} '
                        f'{"Покупка" if stop_order["side"] == "buy" else "Продажа"} {stop_order["exchange"]}.'
                        f'{stop_order["symbol"]} {stop_order["qty"]} @ {stop_order["price"]}'
                    )
        print()
