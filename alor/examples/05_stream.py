...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    order_book = ap_provider.get_order_book(
        exchange, symbol
    )  # Текущий стакан с максимальной глубиной 20 получаем через запрос
    print(order_book)
    if order_book["bids"] and order_book["asks"]:
        print(
            f'bids от {order_book["bids"][0]} до {order_book["bids"][-1]}, asks от {order_book["asks"][0]} до '
            f'{order_book["asks"][-1]}'
        )
    ...
    ap_provider.OnChangeOrderBook = lambda response: print(
        response
    )  # Перехватываем обработку события изменения стакана
    ...
    ap_provider.OnNewQuotes = lambda response: print(
        response
    )  # Перехватываем обработку события прихода новой котировки
