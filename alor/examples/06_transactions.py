...
if __name__ == "__main__":  # Точка входа при запуске этого скрипта
    ...
    )  # Подключаемся к торговому счету. Логин и Refresh Token берутся из файла Config.py
    ...
    portfolio = settings.portfolio_stocks  # Портфель фондового рынка

    # Алор понимает только такой формат фьючерсов. В остальных случаях он возвращает None после постановки заявки.
    # Заявку не ставит
    # symbol = 'SI-9.23'  # Для фьючерсов: <Код тикера>-<Месяц экспирации: 3, 6, 9, 12>.<Две последнии цифры года>
    # symbol = 'RTS-9.23'
    # trade_server_code = FutServerCode  # Торговый сервер фьючерсов
    # portfolio = settings.portfolio_futures  # Портфель фьючерсов
    ...
    response = ap_provider.create_stop_loss_order(
        trade_server_code, account, portfolio, exchange, symbol, "buy", 1, stop_price
    )
    order_id = response["orderNumber"]  # Номер заявки
